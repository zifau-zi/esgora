...
import { LayoutDashboard, ClipboardList, BarChart3, LogOut, User, Menu, X } from 'lucide-react'
...
const NAV_ITEMS = [
  { label: 'Dashboard Siswa', to: '/student/dashboard', icon: LayoutDashboard },
  { label: 'Isi Data ESG', to: '/student/form-esg', icon: ClipboardList },
  { label: 'Hasil Analisis', to: '/student/hasil-analisis', icon: BarChart3 },
]
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-3 px-6 py-6">
        ...
        <div>
          ...
          <span className="ml-2 rounded-md bg-emerald-500/20 px-2 py-0.5 text-[10px] font-bold text-emerald-400">SISWA</span>
        </div>
      </div>
      ...
      <div className="border-t border-white/10 px-4 py-4">
        <div className="mb-3 flex items-center gap-3 px-2">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-white">{user?.name || 'Siswa'}</p>
            <p className="truncate text-xs text-slate-400">{user?.schoolName || 'Sekolah Siswa'}</p>
          </div>
        </div>
        <button
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-64 bg-slate-900">
            ...
            <StudentSidebarContent onNavigate={() => setMobileOpen(false)} />
          </aside>
        </div>
      )}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-16 flex-shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 lg:hidden">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-md bg-emerald-100 px-1.5 py-0.5 text-[10px] font-bold text-emerald-800">SISWA</span>
          </div>
          <button
